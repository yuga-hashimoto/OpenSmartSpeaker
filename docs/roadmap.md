...
- [ ] P19.2 (Priority 4): Termux Bridge Tool (opt-in, advanced) — **core done (this cycle)**:
  ...
  `PreferenceKeys.TERMUX_SHELL_EXECUTE_ENABLED` defaults to false. Hilt
  binds both real classes; the executor joins the composite tool set so
  it's always registered but invisible until opt-in. Tests: 12 cases
  covering all three gate combinations, JSON serialization, non-zero
  exit, timeout, missing command, unknown tool, bridge failure.
  **Still TODO**: Settings UI toggle + "Advanced" group (the preference
  flips via `AppPreferences.set` only for now — no UI switch), spoken
  confirmation layer (`"Run this shell command? yes/no"`), and
  `adb connect localhost:5555` self-control variant. The real
